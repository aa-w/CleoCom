...
{
  CleO.Start();
  CleO.StringExt(FONT_MEDIUM, 400, 240, WHITE, MM, 0, 0, msg);
  CleO.Show();
}
...
void setup()
{
  ...
  if(module == TARGET_CLEO) {
    int h = CleO.FOpen("System/flash50", FILE_READ);
...
void loop()
{
  ...
  CleO.StringExt(FONT_MEDIUM, 400, 240, WHITE, MM, 0, 0, "Load complete");
  if (dur)
