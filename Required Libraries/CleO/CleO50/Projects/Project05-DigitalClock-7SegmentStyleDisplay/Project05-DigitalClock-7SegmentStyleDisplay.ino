...
/**
  Sketch:              
    Project05-DigitalClock-7SegmentStyleDisplay.ino
  Purpose:              
    To desmonstrate how to draw a digital clock using the seven segment font
  External component:   
    PCF8523T RTC module
    CleO Speaker
  Hardware Connection:
...
void updateTimer() {
  ...
  CleO.StringExt(font, 400, 240 - 150, color, MM, 0, 0, buf);

  /* Draw month and day of month */
  sprintf(buf, "%02d    %02d", dayOfMonth, mon);
  CleO.StringExt(font, 400, 240, color, MM, 0, 0, buf);

  /* Draw year */
  sprintf(buf, "%d", y);
  CleO.StringExt(font, 400, 240 + 140, color, MM, 0, 0, buf);
