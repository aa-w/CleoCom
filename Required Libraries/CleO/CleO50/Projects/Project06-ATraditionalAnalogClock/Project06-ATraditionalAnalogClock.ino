...
void drawClockDigits() {
  ...
  for (int i = 0; i < 12; ++i) {
    ...
    CleO.StringExt(font, CLOCK_X, CLOCK_Y, BLACK, MM, angle, RADIUS - 50, digits[i]);
    angle += 30;
  }
}
...
void drawClockMinuteLines() {
  ...
  for (int deg = 0; deg < 360; deg += 6)
  {
    ...
    CleO.LineR1R2(CLOCK_X, CLOCK_Y, radius1, radius2, deg);
  }
}
...
void drawClockHands(int h, int m, int s) {
  ...
  CleO.NeedleWidth(16);
  CleO.NeedleExt(CLOCK_X, CLOCK_Y, RADIUS - 120, BLACK, hour_degrees, 0);

  CleO.NeedleWidth(12);
  CleO.NeedleExt(CLOCK_X, CLOCK_Y, RADIUS - 95, BLACK, min_degrees, 0);

  CleO.NeedleWidth(8);
  CleO.NeedleExt(CLOCK_X, CLOCK_Y, RADIUS - 100, DARK_RED, sec_degrees, 0);
}

void drawClock(int h, int m, int sec) {
  CleO.PointExt(CLOCK_X, CLOCK_Y, RADIUS, WHITE, 0, 0);
  drawClockDigits();
